...
import { TcpOversizedPayloadError, TcpProtocolError } from './tcp-protocol-error.js';
...
describe('TcpFrameEncoder', () => {
  ...
  it('should throw for oversized payload', () => {
    ...
    expect(() => encoder.serialize({ data: '12345678901' })).toThrow(TcpOversizedPayloadError);
  });
});
...
describe('TcpFrameDecoder', () => {
  ...
  it('should handle multiple frames in one chunk', () => {
    ...
    const msg1 = { a: 1 };
    const msg2 = { b: 2 };
    const combined = Buffer.concat([encoder.serialize(msg1), encoder.serialize(msg2)]);
  ...
  it('should handle multiple messages in one TCP packet with split', () => {
    const encoder = new TcpFrameEncoder();
    const decoder = new TcpFrameDecoder();
    const messages = [{ i: 0 }, { i: 1 }, { i: 2 }].map((m) => encoder.serialize(m));
