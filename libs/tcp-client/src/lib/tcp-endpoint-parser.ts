...
export class TcpEndpointParser {
  parse(rawEndpoint: string): TcpEndpoint {
    if (!rawEndpoint || typeof rawEndpoint !== 'string' || rawEndpoint.trim() === '') {
      throw new InvalidTcpEndpointError('Invalid TCP endpoint: missing endpoint\n\nExpected format host:port e.g. localhost:4000 or [::1]:4000');
    }
    ...
    if (trimmedEndpoint.startsWith('[')) {
      ...
      if (closingBracketIndex === -1) {
        throw new InvalidTcpEndpointError(`Invalid TCP endpoint: ${trimmedEndpoint}\n\nMissing closing bracket for IPv6 address. Expected format [::1]:4000`);
      }
    ...
    if (!Number.isFinite(port) || !Number.isInteger(port) || String(port) !== portString) {
      throw new InvalidTcpPortError(`Invalid TCP endpoint: ${trimmedEndpoint}\n\nPort must be a number between 1 and 65535.`);
    }
