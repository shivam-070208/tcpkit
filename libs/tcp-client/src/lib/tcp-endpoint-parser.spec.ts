...
describe('TcpEndpointParser', () => {
  ...
  it('should parse localhost:4000', () => {
    expect(parser.parse('localhost:4000')).toEqual({
      host: 'localhost',
      port: 4000,
    });
  });
  ...
  it('should parse 127.0.0.1:4000', () => {
    expect(parser.parse('127.0.0.1:4000')).toEqual({
      host: '127.0.0.1',
      port: 4000,
    });
  });
  ...
  it('should parse 192.168.1.20:8080', () => {
    expect(parser.parse('192.168.1.20:8080')).toEqual({
      host: '192.168.1.20',
      port: 8080,
    });
  });
  ...
  it('should parse my-service.local:5000', () => {
    expect(parser.parse('my-service.local:5000')).toEqual({
  ...
  it('should parse [2001:db8::1]:4000', () => {
    expect(parser.parse('[2001:db8::1]:4000')).toEqual({
      host: '2001:db8::1',
      port: 4000,
    });
  });
  ...
  it('should reject invalid hostname', () => {
    expect(() => parser.parse('invalid_host:4000')).toThrow(
      InvalidTcpEndpointError,
    );
  });
});
