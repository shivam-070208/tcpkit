  ...
  async connect(): Promise<Socket> {
    ...
    this.pendingConnectionPromise = new Promise((resolve, reject) => {
      const socketTimeoutId = setTimeout(() => {
        ...
        this.connectionError = new TcpConnectionTimeoutError(
          this.tcpEndpoint.host,
          this.tcpEndpoint.port,
      ...
      const onError = (socketError: Error) => {
        cleanup();
        this.connectionState = TcpConnectionState.ERROR;
        this.connectionError = new TcpConnectionError(
          `Failed to connect to ${this.tcpEndpoint.host}:${this.tcpEndpoint.port} - ${socketError.message}`,
        );
        this.pendingConnectionPromise = null;
        reject(this.connectionError);
      };
